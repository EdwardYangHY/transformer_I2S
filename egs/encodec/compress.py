...
def decompress_from_frames(model: EncodecModel, frames: list):
    if type(frames) == list:
        frames = frames
    elif type(frames) == torch.Tensor:
        frames = [(frames, None)]
    wav_length = int(frames[0][0].shape[-1] * 320)
    fo = io.BytesIO()
    compress_from_frames(model, frames, wav_length, fo)
    compressed = fo.getvalue()
    return decompress(compressed)

def compress_from_frames(model: EncodecModel, frames: list, wav_length: int, fo):
    wav_length = wav_length
    use_lm = False
    metadata = {
        'm': model.name,                 # model name
        'al': wav_length,             # audio_length
