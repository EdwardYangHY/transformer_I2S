...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, att_mask=None):
        ...
        if self.use_gx:
            ...
            gx = attn_windows[:, -1, :].view(-1, self.nW, C).mean(1)
            # [B*nW, Ww*Wh, C]
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, embed_dim=512, window_size=(12, 12), num_heads=8, 
        ...
        # In later version torch.meshgrid will require indexing, or default will be indexing="xy"
        # Now torch.meshgrid(*tensors) == torch.meshgrid(*tensors, indexing='ij')
