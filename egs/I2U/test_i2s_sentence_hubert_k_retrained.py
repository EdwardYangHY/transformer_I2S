...
def main(model_path):
    ...
    i2u_model = TransformerVAEwithCNN_Modified(
    ...
    i2u_model.load_state_dict(torch.load(model_checkpoint)["model_state_dict"])
    i2u_model.eval()
    i2u_model.to(device)
    
    I_want_acc, gt_list, ans_list = test(i2u_model, model_config)

    with open(model_path + f"/val_recognition_results_sentence.txt", "w") as f:
