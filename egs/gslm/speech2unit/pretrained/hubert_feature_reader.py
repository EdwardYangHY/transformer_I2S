...
class HubertFeatureReader:
    ...
    def read_audio_(self, path, ref_len=None, channel_id=None):
        #wav, sr = sf.read(path, samplerate = self.task.cfg.sample_rate)
    ...
    def get_feats_(self, file_path, ref_len=None, channel_id=None):
        x = self.read_audio_(file_path, ref_len, channel_id)
        with torch.no_grad():
            x = torch.from_numpy(x).float()
            if self.use_cuda:
                x = x.cuda()
