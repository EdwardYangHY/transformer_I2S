...
def main(args):
    ...
    models, _model_args = checkpoint_utils.load_model_ensemble(
        args.common_eval.path.split(os.pathsep), # List: [pt's path]
        arg_overrides=overrides,                 # dict: {}
        task=task,                               # fairseq 自定义的 task 类
        suffix=getattr(args, "checkpoint_suffix", ""), # ''
    )

    # with open("fairseq_uLM_big_state_dict.txt", "w") as f:
    ...
    for batch in make_batches(prompts, args, task, max_positions):
        ...
        results = []
        # this is the steps using the model
