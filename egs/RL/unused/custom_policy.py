...
class CustomActorAlpha(BasePolicy):
    """
    Actor network (policy) for TD3.
    :param observation_space: Obervation space
    :param action_space: Action space
    ...
    def __init__(
        ...
        # Deterministic action
        # self.mu = nn.Sequential(*actor_net)
        self.mu = CustomMuAlpha(net_arch, activation_fn)
...
class CustomMu(nn.Module):
    ...
    def forward(self, x):
        ...
        # alpha = F.gumbel_softmax(alpha, hard=True)
        ...
        if self.use_embed:
            action = th.cat([image_features, embed], dim=1)
        else:
            action = image_features
        return action


class CustomMuAlpha(nn.Module):
    def __init__(self, net_arch: List[List[int]], activation_fn: Type[nn.Module]):
        super().__init__()
        assert len(net_arch) == 2 and len(net_arch[0]) == len(net_arch[1]) == 3 and net_arch[0][0] == net_arch[1][0] and net_arch[0][2] == 2
        self.fc1 = nn.Linear(net_arch[0][0], net_arch[0][1])
    ...
    def forward(self, x):
        ...
        if self.soft_image_features and self.training:
            ...
            index = th.argmax(alpha, dim=1, keepdim=False)  # (batch_size, 1)
            alpha = F.one_hot(index, num_classes=2)
        
        action = th.cat([alpha, embed], dim=1)
        return action
...
class CustomTD3PolicyAlpha(BasePolicy):
    """
    Policy class (with both actor and critic) for TD3.
    :param observation_space: Observation space
    :param action_space: Action space
    ...
    def make_actor(self, features_extractor: Optional[BaseFeaturesExtractor] = None) -> CustomActorAlpha:
        actor_kwargs = self._update_features_extractor(self.actor_kwargs, features_extractor)
        return CustomActorAlpha(**actor_kwargs).to(self.device)
    ...
    def _sample_action(
        ...
        if isinstance(self.action_space, gym.spaces.Box):
            ...
            if action_noise is not None:
                ...
                print("w/o action noise", unscaled_action[:,-8:])
                unscaled_action = unscaled_action + action_noise().astype(unscaled_action.dtype)
                print("w/  action noise", unscaled_action[:,-8:])
