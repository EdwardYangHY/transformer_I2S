...
sentence_encoder.load_state_dict(torch.load(config["u2u"]["path2_incremental"]))
...
# tacotron2
checkpoint_path = config["u2s"]["tacotron2_incremental"]
...
if __name__ == "__main__":
    action_noise = NormalActionNoise(
        mean=np.zeros(768+config["u2u"]["d_embed"]),
        sigma=np.concatenate([np.zeros(768), 0.3*np.ones(config["u2u"]["d_embed"])]),
        )
    ...
    model2 = CustomDDPG.load("./logs_ddpg_embed_icassp/best_model.zip", env=env)
    model2.use_embed()
    ...
    model2.learn(
        ...
        eval_log_path="./logs_ddpg_embed_icassp_incremental/",
        )
    
    eval_env.save_rewards("./logs_ddpg_embed_icassp_incremental/", data_name=datetime.datetime.now().strftime('%y%m%d%H%M')+"_ddpg")