...
model_path = "../../saved_model/I2U/VC_5_captions/7*7_img_1024*16*12_99accuracy/bleu-4_BEST_checkpoint_coco_5_cap_per_img_1_min_word_freq_gpu.pth.tar"
...
sentence_encoder = TransformerConditionedLM(**model_params)
...
def i2u2s2t_img(img):
    ...
    seq = sentence_encoder.decode(img, word_map['<start>'], word_map['<end>'], max_len=130, beam_size=5)
    # img, _ = sentence_encoder.image_encoder(img)
...
class SpoLacq(gym.Env):
    def __init__(
        ...
        # self.make_food_storage_decode(img_list)
    ...
    def make_food_storage(self, img_list: list):
        ...
        for img_path in tqdm(img_list):
            ...
            image_decode = np.array(Image.fromarray(img).resize((256, 256)))
            image_decode = image_decode.transpose(2, 0, 1)
            assert image_decode.shape == (3, 256, 256)
            assert np.max(image_decode) <= 255
            # image = Image.open(img_path)
            # image = np.asarray(image)
            self.food_storage.append(image)
            self.food_storage_decode.append(image_decode)
    ...
    # def make_food_storage_decode(self, img_list: list):
    #     self.food_storage_decode = list()
    #     for img_path in tqdm(img_list):
    #         img = imread(img_path)
    #         if len(img.shape) == 2:
    ...
    #         # image = Image.open(img_path)
    #         # image = np.asarray(image)
    #         self.food_storage_decode.append(image)
    
    def get_transformed_image_decode(self, data_num):
        transform = transforms.Normalize(mean=self.mean, std=self.std)
        image = self.food_storage_decode[data_num]
        # image = image.transpose(2, 0, 1)
        image = torch.FloatTensor(image / 255.)
        # image = transform(image)
        # image = image.numpy()
    ...
    def reward(self, action):
        ...
        # this should not give action. give raw images instead
        img = self.get_transformed_image_decode(pred_num)
        transcription = i2u2s2t_img(img)
        # transcription = i2u2s2t(action[:-2])
...
if __name__ == "__main__":
    ...
    model2 = CustomDDPG(
        ...
        tensorboard_log='./spolacq_tmplog_record_choosepos/',
        # policy_kwargs=dict(
        #     net_arch=dict(
    ...
    model2.learn(
        ...
        eval_log_path="./logs_ddpg_record_choosepos/",
        )
    
    env.save_rewards("./spolacq_tmplog_record_choosepos/rl_accuracy.npy")