...
class SpoLacq(gym.Env):
    ...
    def reward(self, action):
        ...
        ans_num_2 = self.data_num2 if ans == 0 else self.data_num1

        alpha = action[-2:]
        # ------------------------------------------------------------------------------------------
        # transcription = i2u2s2t(action[:-2])
        # ------------------------------------------------------------------------------------------
        # self.food_storage[self.data_num1]
        ...
        img_path_2 = self.img_list[ans_num_2]
        # img_path = img_path.split("/")[-1]
        # right_ans, right_name = judge_ans(transcription, img_path)
        right_ans, right_name = judge_ans(transcription, img_path)
        right_ans_2, right_name_2 = judge_ans(transcription, img_path_2)
        print(
            self.num_step,
...
if __name__ == "__main__":
    ...
    model2 = CustomDDPG(
        ...
        tensorboard_log='./spolacq_tmplog_256_hubert_no_sentence_sanity_pos/',
        # policy_kwargs=dict(
        #     net_arch=dict(
    ...
    model2.learn(
        ...
        eval_log_path="./logs_ddpg_256_hubert_no_sentence_sanity_pos/",
        )
    
    env.save_rewards("./spolacq_tmplog_256_hubert_no_sentence_sanity_pos/rl_accuracy.npy")