...
class CustomFeaturesExtractorCNN(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Dict):
        super().__init__(observation_space, 150)

        self.cnn = ResNet50()
        # self.visual_net = ResNet50()
        self.state_net = nn.Linear(observation_space.spaces["state"].shape[0], 2048)
    ...
    def forward(self, observation: Dict[str, th.Tensor]):
        ...
        # leftmean_features = th.mean(self.visual_net(observation["leftimage"]), dim=1)
        # rightmean_features = th.mean(self.visual_net(observation["rightimage"]), dim=1)
...
class CustomContinuousCriticCNN(BaseModel):
    ...
    def __init__(
        ...
        for idx in range(n_critics):
            ...
            q_net = QNetCNN(net_arch, activation_fn)
...
class QNetCNN(nn.Module):
    def __init__(self, net_arch: List[int], activation_fn: Type[nn.Module]):
        ...
        # self.fc_img_feat = nn.Linear(2048, 50)
    
    def forward(self, x):
        f = x[:, :2048*3]
        img_feat = x[:, 2048*3:2048*3+49*2048]
        if self.use_embed:
            embed = x[:, 2048*3+49*2048:-2]
        alpha = x[:, -2:]

        img_feat = img_feat.view(-1, 49, 2048)
        img_feat = th.mean(img_feat, dim=1)
        # img_feat = self.fc_img_feat(img_feat)  # (batch, 50)
...
class CustomFeaturesExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Dict):
        ...
        self.state_net = nn.Linear(observation_space.spaces["state"].shape[0], 768)
    ...
    def forward(self, observation: Dict[str, th.Tensor]):
        ...
        with th.no_grad():
            self.vit.eval()
            leftimage_features = self.vit(observation["leftimage"])
            rightimage_features = self.vit(observation["rightimage"])
        
        # features = th.cat((state, self.fc(leftimage_features), self.fc(rightimage_features)), dim=1)
        # features = self.relu(features)
        features = th.cat((state, leftimage_features, rightimage_features), dim=1)
...
class CustomMu(nn.Module):
    def __init__(self, net_arch: List[List[int]], activation_fn: Type[nn.Module]):
        ...
        self.fc2 = nn.Linear(net_arch[0][1], net_arch[0][2], bias=False)
    ...
    def forward(self, x):
        ...
        alpha = F.gumbel_softmax(alpha, tau=10.)
...
class CustomDDPG(TD3):
    ...
    def _sample_action(
        ...
        if isinstance(self.action_space, gym.spaces.Box):
            ...
            if action_noise is not None:
                ...
                print("w/o action noise", unscaled_action[:,-(self.policy.actor.mu.layernorm.weight.size(0)+2):])
                unscaled_action = unscaled_action + action_noise().astype(unscaled_action.dtype)
                unscaled_action = np.concatenate([
                    unscaled_action[:,:-(self.policy.actor.mu.layernorm.weight.size(0)+2)],
                    np.clip(unscaled_action[:,-(self.policy.actor.mu.layernorm.weight.size(0)+2):-2], -2.5, 2.5),
                    unscaled_action[:,-2:]
                ], axis=1)
                print("w/  action noise", unscaled_action[:,-(self.policy.actor.mu.layernorm.weight.size(0)+2):])
    ...
    # def load_param_from_dqn(self, model):
    #     self.policy.actor.features_extractor.load_state_dict(model.policy.q_net.features_extractor.state_dict())
    #     self.policy.critic.features_extractor.load_state_dict(model.policy.q_net.features_extractor.state_dict())
    #     self.policy.actor.mu.fc1.load_state_dict(model.policy.q_net.q_net.fc1.state_dict())
    #     self.policy.actor.mu.fc2.load_state_dict(model.policy.q_net.q_net.fc2.state_dict())
...
class CustomQNetwork(QNetwork):
    def __init__(self, *args, **kwargs):
        ...
        self.q_net = CustomQNet()


class CustomQNet(nn.Module):
    def __init__(self, features_dim: int = 150):
        super().__init__()
    ...
    def forward(self, x):
        ...
        return F.softmax(self.fc2(features), dim=1)